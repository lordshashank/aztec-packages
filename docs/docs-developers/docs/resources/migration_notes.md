...
The handshake registry defaults to the standard registry; a custom registry can be configured with `via_handshake(registry)`. Contracts whose senders are not set up for registry handshakes should use `MessageDelivery::onchain_unconstrained()` instead, which keeps the wallet-driven tagging behavior.
