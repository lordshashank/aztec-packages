...
template <typename FF_> class CircuitBuilderBase {
  ...
  public:
     ...
     * been copied into the prover polynomials and the copy cycles have been computed (after which
     * `variables`/`real_variable_index` are no longer needed). The builder is left in a valid-to-destroy but
     * unusable-for-circuit-construction state.
