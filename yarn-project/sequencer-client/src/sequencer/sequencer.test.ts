...
describe('sequencer', () => {
  ...
  describe('config updates', () => {
    it('rejects a config with sub-minimum allocation multipliers without committing it', () => {
      ...
      // the DA multiplier too so the DA admission dimension binds regardless of the (effectively unbounded)
      // mana limit this test uses.
      expect(() =>
        sequencer.updateConfig({ perBlockAllocationMultiplier: 0.5, perBlockDAAllocationMultiplier: 0.5 }),
      ).toThrow(/per-block allocation multipliers are below the network admission limit/);
